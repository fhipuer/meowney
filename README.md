...
- 대시보드 레짐 카드를 저장된 경량 요약 API로 전환해 초기 화면을 막던 전체 레짐 재계산 제거
- 레짐 시계열 계산의 반복 날짜 파싱을 제거하고 CPU 작업을 스레드풀로 격리해 다른 API 정체 방지
- 리밸런싱 알림의 초기 기본값 중복 요청을 제거하고 JSON·정적 자산 gzip 압축 적용

## v1.20.2 주요 변경

- 자산별 배분 차트 툴팁의 항목·라벨 글자색을 다크 테마 전경색으로 고정해 가독성 개선

## v1.20.1 주요 변경

- NAS에서 `/assets` 주소를 직접 열거나 새로고침할 때 Vite 정적 폴더와 충돌해 404가 발생하던 SPA 라우팅 수정
