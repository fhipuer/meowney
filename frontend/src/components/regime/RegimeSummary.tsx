    ...
    <Card>
      <CardContent className="flex flex-wrap items-center gap-4 p-5">
        ...
        <div className="min-w-40">
          ...
          <p className={`text-xl font-semibold ${isLoading ? 'text-foreground' : reviewTone}`}>{isLoading ? '확인 중' : data?.needs_new_review ? '상세 점검 필요' : data?.review_acknowledged ? '점검 완료' : data?.review_urgency === 'watch' ? '관찰 필요' : '점검 불필요'}</p>
        </div>
        <div className="flex flex-1 flex-wrap items-center gap-2 text-sm text-muted-foreground">
          {data ? (
            <>
              <Badge variant={TONE_STYLES[regimeLevelTone(data.automatic_regime)].badge}>확정 {data.automatic_regime}</Badge>
              <span className={data.triggers.length ? 'text-danger' : undefined}>활성 경보 {data.triggers.length}개</span>
            </>
