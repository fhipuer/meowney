    ...
    <Card>
      <CardContent className="flex flex-wrap items-center gap-4 p-5">
        ...
        <div className="min-w-40">
          ...
          <p className={`text-xl font-semibold ${isLoading ? 'text-foreground' : reviewTone}`}>{isLoading ? '확인 중' : data?.needs_new_review ? '상세 점검 필요' : data?.review_acknowledged ? '마지막 신호 확인 완료' : data?.review_urgency === 'watch' ? '다음 발표까지 관찰' : '새 점검 사유 없음'}</p>
        </div>
