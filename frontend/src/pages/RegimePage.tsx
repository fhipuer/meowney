...
import type { RegimeLevel, RegimeSignal, RegimeSnapshot, ReviewUrgency } from '@/types'

const LEVELS: RegimeLevel[] = ['유지', '경계', '약화', '전환']
const DOMAIN_TABS = [
  { id: 'market', label: '시장' }, { id: 'growth', label: '성장·고용' },
  { id: 'inflation', label: '물가' }, { id: 'rates', label: '금리' },
...
const signalStatusLabel: Record<string, string> = {
  강함: '개선 신호', 중립: '임계 변화 없음', 둔화: '악화 접근', 약화: '악화 신호', unavailable: '미수집',
}
const formatDate = (value: string) => value.slice(2, 7).replace('-', '.')
const formatNumber = (value: number) => Math.abs(value) >= 1000
  ...
  return <Card className="overflow-hidden">
    ...
    <CardContent className="pt-3">
      {history.length > 1 ? <div className="h-56 w-full">
        <ResponsiveContainer width="100%" height="100%"><LineChart data={history} margin={{ top: 8, right: 12, bottom: 8, left: 4 }}>
...
function RateComparison({ signals }: { signals: RegimeSignal[] }) {
  const selected = signals.filter(signal => ['us10y', 'tips10y', 'bei10y'].includes(signal.id))
  const byDate = new Map<string, Record<string, string | number>>()
  selected.forEach(signal => signal.history?.forEach(point => {
    byDate.set(point.date, { ...(byDate.get(point.date) || { date: point.date }), [signal.id]: point.value })
  }))
...
function JudgmentEditor({ snapshot }: { snapshot: RegimeSnapshot }) {
  ...
  const [level, setLevel] = useState<RegimeLevel | ''>(snapshot.user_regime || '')
  const [note, setNote] = useState(snapshot.user_note || '')
  const mutation = useMutation({ mutationFn: () => regimeApi.updateSnapshot(snapshot.id, { user_regime: level || undefined, user_note: note || undefined }), onSuccess: () => client.invalidateQueries({ queryKey: ['regime', 'history'] }) })
  return <div className="mt-5 grid gap-3 md:grid-cols-[160px_1fr_auto]"><select className="h-10 rounded-md border bg-background px-3 text-sm" value={level} onChange={e => setLevel(e.target.value as RegimeLevel | '')}><option value="">내 판정 미입력</option>{LEVELS.map(item => <option key={item}>{item}</option>)}</select><Textarea value={note} onChange={e => setNote(e.target.value)} placeholder="판단 메모 (선택)" className="min-h-10" /><Button variant="outline" onClick={() => mutation.mutate()} disabled={mutation.isPending}>저장</Button></div>
}
...
export function RegimePage() {
  ...
  const current = useQuery({ queryKey: ['regime', 'current'], queryFn: regimeApi.getCurrent })
  const history = useQuery({ queryKey: ['regime', 'history'], queryFn: regimeApi.getHistory })
  const refresh = useMutation({ mutationFn: regimeApi.refresh, onSuccess: () => client.invalidateQueries({ queryKey: ['regime'] }) })
  const snapshot = useMutation({ mutationFn: () => regimeApi.createSnapshot({ user_regime: judgment || undefined, user_note: note || undefined, review_completed: reviewCompleted }), onSuccess: () => { setNote(''); setReviewCompleted(false); client.invalidateQueries({ queryKey: ['regime'] }) } })
  ...
  return <div className="space-y-8 pb-14 pt-3">
    ...
    <Tabs defaultValue="current" className="space-y-6"><TabsList><TabsTrigger value="current">현재</TabsTrigger><TabsTrigger value="indicators">지표</TabsTrigger><TabsTrigger value="history">History</TabsTrigger></TabsList>
      <TabsContent value="current" className="mt-0 space-y-8 pt-3">
        {data ? <RegimeCurrentOverview data={data} judgment={judgment} note={note} reviewCompleted={reviewCompleted} snapshotPending={snapshot.isPending} onJudgment={setJudgment} onNote={setNote} onReviewCompleted={setReviewCompleted} onSnapshot={() => snapshot.mutate()} /> : <Card><CardContent className="p-8 text-muted-foreground">판정 데이터를 불러오는 중입니다.</CardContent></Card>}
      </TabsContent>

      <TabsContent value="indicators" className="mt-0"><Tabs defaultValue="market" className="space-y-6"><div className="sticky top-16 z-10 -mx-1 overflow-x-auto bg-background/95 px-1 py-3 backdrop-blur"><TabsList className="w-max">{DOMAIN_TABS.map(tab => <TabsTrigger key={tab.id} value={tab.id}>{tab.label}</TabsTrigger>)}</TabsList></div><TabsContent value="market" className="mt-0 space-y-6"><MarketIndicators signals={data?.signals.filter(signal => signal.id.startsWith('market_')) || []} fetchedAt={data?.data_quality.last_fetched_at} /></TabsContent>{DOMAIN_TABS.filter(tab => tab.id !== 'market').map(tab => { const signals = data?.signals.filter(signal => signal.domain === tab.id) || []; return <TabsContent key={tab.id} value={tab.id} className="mt-0 space-y-6"><div className="px-1"><h2 className="text-xl font-semibold">{tab.label}</h2><p className="mt-1 text-sm text-muted-foreground">{signals.length}개 지표 · 최신 관측값과 변화 구간을 함께 표시합니다.</p></div>{tab.id === 'rates' && <RateComparison signals={signals} />}<div className="grid gap-5 xl:grid-cols-2">{signals.map(signal => <SignalCard key={signal.id} signal={signal} />)}</div></TabsContent> })}</Tabs></TabsContent>
      <TabsContent value="history" className="mt-0 space-y-4">{history.data?.length ? history.data.map(item => <Card key={item.id}><CardContent className="p-6"><div className="flex flex-wrap items-center gap-3"><span className="font-medium">{new Date(item.created_at).toLocaleString('ko-KR')}</span><Badge className={levelClass[item.automatic_regime]}>자동 {item.automatic_regime}</Badge><Badge variant="outline">사용자 {item.user_regime || '미입력'}</Badge>{item.review_urgency && <Badge variant="secondary">{urgencyLabel[item.review_urgency]}</Badge>}</div><p className="mt-3 text-sm text-muted-foreground">{item.reasons?.slice(0, 3).join(' · ') || '주요 악화 사유 없음'}</p><JudgmentEditor snapshot={item} /></CardContent></Card>) : <Card><CardContent className="p-8 text-center text-muted-foreground">아직 공식 Snapshot이 없습니다.</CardContent></Card>}</TabsContent>
    </Tabs>
  </div>
}
